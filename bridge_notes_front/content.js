...
class BRIDGENotesCapture {
  ...
  async extractTextBetweenAsync(start, end) {
    if (!start || !end) return "";

    // 스크롤 처리: 선택 범위 내 미로드 콘텐츠 로드
    await this.ensureContentLoaded(start, end);

    // 시작과 끝 요소 사이의 모든 요소 수집
    const elements = [];
    ...
    elements.forEach((element) => {
      ...
      if (messageText) {
        ...
        formattedText += `${role}: ${messageText}\n\n`;
      }
    });
  ...
  async ensureContentLoaded(start, end) {
    // 긴 대화의 스크롤 처리
    // Lazy loading으로 로드되지 않은 콘텐츠를 로드

    const startRect = start.getBoundingClientRect();
    const endRect = end.getBoundingClientRect();

    // 시작과 끝 요소가 화면에 보이는지 확인
    const isStartVisible =
      startRect.top >= 0 && startRect.bottom <= window.innerHeight;
    const isEndVisible =
      endRect.top >= 0 && endRect.bottom <= window.innerHeight;

    if (!isStartVisible || !isEndVisible) {
      // 스크롤하여 콘텐츠 로드
      start.scrollIntoView({ behavior: "smooth", block: "center" });
      await this.delay(500); // 로딩 대기
