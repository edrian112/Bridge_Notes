...
**구현 내용**:
- `content.js`의 `extractTextFromRange()`에서 각 메시지별 역할 감지 및 구조화된 형식으로 반환
- `ResultArea.js`에서 `formatOriginalText()`, `parseMessages()` 함수 추가하여 HTML 포맷팅
...
**구현 내용**:
- 동적 언어 전환을 위한 JavaScript 기반 i18n 시스템 구축
- `scripts/i18n/translations.js` - 한국어/영어 번역 파일
- `scripts/i18n/i18n.js` - i18n 유틸리티 클래스 (싱글톤)
- HTML 요소에 `data-i18n`, `data-i18n-placeholder`, `data-i18n-title` 속성 추가
- 설정 모달에서 언어 선택 시 실시간 UI 번역 적용
- 토스트 메시지, 확인 대화상자 등 모두 i18n 지원

**관련 파일**:
- `bridge_notes_extension/scripts/i18n/translations.js` - 번역 데이터
- `bridge_notes_extension/scripts/i18n/i18n.js` - i18n 유틸리티
- `bridge_notes_extension/sidepanel.html` - data-i18n 속성 추가
- `bridge_notes_extension/scripts/components/Settings.js` - 언어 변경 로직
...
### ✅ 영어모드 지원 (2025-12-12)
- JavaScript 기반 동적 i18n 시스템 구축
- 한국어/영어 번역 파일 생성 (translations.js)
- 설정에서 언어 선택 시 실시간 UI 번역 적용
- data-i18n 속성으로 모든 UI 텍스트 번역 지원

### ✅ AI/사용자 대화 구분 (2025-12-12)
- 오리지널 텍스트 영역에서 AI와 사용자 대화 시각적 구분
- 메시지별 역할 감지 및 구조화된 형식 반환
- 사용자/AI 메시지에 각각 다른 배경색 및 테두리 스타일
- 라이트/다크 모드 지원
...
- PHASE2_PLAN.md: Phase 2 기능 계획
- claudedocs/CSS_ARCHITECTURE.md: CSS 구조 문서
