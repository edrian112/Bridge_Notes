...
export class History {
  ...
  initEventListeners() {
    ...
    window.addEventListener('planUpdated', async (event) => {
      ...
      await this.render(); // 플랜 변경 시 히스토리 목록 재렌더링
    });
  }
  ...
  async render() {
    try {
      ...
      if (this.toast) {
        this.toast.error(i18n.t('toast.historyLoadFailed'));
      }
    }
  }
  ...
  async handleDeleteItem(id) {
    try {
      ...
      if (this.toast) {
        this.toast.success(i18n.t('toast.historyItemDeleted'));
      }
    } catch (error) {
      console.error('Delete item error:', error);
      if (this.toast) {
        this.toast.error(i18n.t('toast.historyDeleteFailed'));
      }
    }
  }
  ...
  async handleClearHistory() {
    ...
    try {
      ...
      if (this.toast) {
        this.toast.success(i18n.t('toast.historyAllDeleted'));
      }
    } catch (error) {
