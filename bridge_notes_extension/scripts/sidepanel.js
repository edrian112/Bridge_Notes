...
class BRIDGENotesSidePanel {
  ...
  async init() {
    ...
    window.addEventListener('planUpdated', async () => {
  ...
  async startCapture() {
    try {
      ...
      this.toast.success(i18n.t("toast.selectionModeActivated"));
    } catch (error) {
  ...
  async updateUsageDisplay() {
    try {
      ...
      // Chrome storage에서 요금제 정보 가져오기 (Pricing.js와 동기화)
      const result = await chrome.storage.local.get(["planType", "remainingUsage"]);

      const planType = result.planType || "free";
      const remainingUsage = result.remainingUsage;
      ...
      if (planType === 'free' || planType === 'max') {
        ...
        displayText = remainingUsage !== undefined ? remainingUsage : 0;
      }

      // UI 업데이트
      remainingCountElement.textContent = displayText;

      console.log(`Usage updated: ${planType} - ${displayText} remaining`);
    } catch (error) {
